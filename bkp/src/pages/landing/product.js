function Product(props) {
  return (
    <div className={`h-auto flex  flex-col  m-10 ${props.textColor} md:p-8 md:m-8 p-2`}>
      <h1 className="text-4xl font-bold mb-6">Welcome to the Future of Decentralized Governance</h1>

      {/* Pandao Overview Section */}
      <section>
        <h2 className="text-3xl font-bold mt-4 mb-2">What is Pandao?</h2>
        <p className="mt-2">
          In an era where decentralization is more than a buzzword, our platform represents a revolutionary step forward in empowering communities and organizations. Built on the robust foundation of the Radix blockchain, our platform offers a universal DAO (Decentralized Autonomous Organization) launchpad, designed to democratize governance and innovate how collective decisions are made and implemented.
        </p>
      </section>
      Radix stands out in the blockchain landscape for its unique approach to scalability and security, which are critical for the efficient operation of DAOs. Radix's ledger design and consensus mechanism ensure that as more DAOs are launched and more participants join, the network maintains high transaction speeds without compromising on security. This is crucial for DAOs, where timely and secure execution of governance decisions can be the difference between success and stagnation.
      <section>
        <section>
          <div>
