...
function Header(props) {
    const [darkModeOn, changeDarkMode] = useState(false);
    ...
    const handleNightModeToggle = () => {
        ...
        // Call the onToggle function passed as prop
                ...
                <nav className='md:flex md:flex-row  flex flex-row w-full justify-between p-0 m-0 md:place-items-start'>
                    ...
                    <div className='hidden md:flex  md:basis-2/5 justify-left pt-4 pr-0 '>
                        <div className='flex flex-row justify-center w-full px-2'>
                            <span className={`ppx-2 text-2xl underline font-bold hover:scale-110 transition duration-300 ease-in-out ${props.textColor} `}><Link to='/about-us'>About us</Link></span>
                            <span className={`px-2 text-2xl font-bold ${props.textColor}`}>|</span>
                            <span className={`ppx-2 text-2xl underline font-bold hover:scale-110 transition duration-300 ease-in-out ${props.textColor} `}><Link to='/product'>Products</Link></span>
                            <span className={`px-2 text-2xl font-bold ${props.textColor}`}>|</span>
                            <span className={`px-2 text-2xl underline  font-bold hover:scale-110 transition duration-300 ease-in-out ${props.textColor} `}><Link to="/resource">Resources</Link></span>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <button className={`hidden md:flex bg-gray-800 text-white px-4 py-2 rounded-full ${isNightMode ? 'bg-yellow-500' : 'bg-gray-600'}`} onClick={handleNightModeToggle}>
                                    ...
                                    menuOpen && (
                                        <div class="right-auto  z-10 bg-white  rounded-lg shadow w-44 dark:bg-gray-700 ">
                                            <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownDefaultButton">
                                                ...
                                                <li className='flex flex-row justify-center'>
                                                    <button className={`flex bg-gray-800 text-white px-4 py-2 rounded-full ${isNightMode ? 'bg-yellow-500' : 'bg-gray-600'}`} onClick={handleNightModeToggle}>
                                                        {isNightMode ? 'Day Mode' : 'Night Mode'}
                                                    </button>
                                                </li>
                                            </ul>
